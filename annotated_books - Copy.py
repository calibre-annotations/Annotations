...
class MarkupTableModel(QAbstractTableModel):
    ...
    def setData(self, index, value, role):
        ...
        if col == self.ENABLED_COL:
            if self.arraydata[row][self.ENABLED_COL].checkState() == qCheckState_Checked:
                self.arraydata[row][self.ENABLED_COL].setCheckState(qCheckState_Unchecked)
            else:
                self.arraydata[row][self.ENABLED_COL].setCheckState(qCheckState_Checked)
    ...
    def sort(self, Ncol, order=qSortOrder_AscendingOrder):
        ...
        if Ncol == self.ENABLED_COL: # Don't sort on the checkbox column.
            self.arraydata = sorted(self.arraydata, key=lambda row: row[Ncol].checkState() == qCheckState_Checked, reverse=(order == qSortOrder_DescendingOrder))
        else:
